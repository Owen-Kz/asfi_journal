...
function renderSpecialIssues($con, $page = 1, $filters = []) {
    $items_per_page = 6;
    ...
    try {
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            if ($totalPages > 1) {
                ...
                if ($page > 1) {
                    $prevUrl = buildPaginationUrlSI($filters, $page - 1);
                ...
                if ($startPage > 1) {
                    $firstUrl = buildPaginationUrlSI($filters, 1);
                ...
                for ($i = $startPage; $i <= $endPage; $i++) {
                    ...
                    $pageUrl = buildPaginationUrlSI($filters, $i);
                ...
                if ($endPage < $totalPages) {
                    ...
                    $lastUrl = buildPaginationUrlSI($filters, $totalPages);
                ...
                if ($page < $totalPages) {
                    $nextUrl = buildPaginationUrlSI($filters, $page + 1);
...
function buildPaginationUrlSI($filters, $page) {
    $params = [];
...
// If called directly, render Special Issues with GET parameters
