 ...
 * Filters articles where article_type is 'Career Corner' or 'Learning Corner' (case-insensitive)
 */
...
function renderArticleLC($row, $authorsName) {
    ...
    $careerCornerBadge = '<span class="career-corner-badge inline-flex items-center gap-1 text-[11px] md:text-sm text-orange-700 bg-orange-50 px-1.5 md:px-2 py-0.5 rounded-full whitespace-nowrap">
    ...
    $learningCornerBadge = '<span class="learning-corner-badge inline-flex items-center gap-1 text-[11px] md:text-sm text-blue-700 bg-blue-50 px-1.5 md:px-2 py-0.5 rounded-full whitespace-nowrap">
        <svg width="14" height="14" class="md:w-4 md:h-4" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M12 6.042A8.967 8.967 0 006 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 016 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 016-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0018 18a8.967 8.967 0 00-6 2.292m0-14.25v14.25" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
        </svg>
        Learning Corner
    ...
    // Determine which badge to show based on article type (case-insensitive)
    $articleTypeLower = strtolower($row['article_type']);
    $cornerBadge = '';
    if (strpos($articleTypeLower, 'career') !== false) {
        $cornerBadge = $careerCornerBadge;
    } elseif (strpos($articleTypeLower, 'learning') !== false) {
        $cornerBadge = $learningCornerBadge;
    }
...
function renderCareerCorner($con, $page = 1, $filters = []) {
    ...
    // Build WHERE clause - Case-insensitive filter for Career Corner AND Learning Corner
    $whereClauses = ["`journals`.`is_publication` = 'yes'"];
    
    // Case-insensitive article type filter using UPPER() for consistency
    $whereClauses[] = "(UPPER(`journals`.`article_type`) = 'CAREER CORNER' OR UPPER(`journals`.`article_type`) = 'LEARNING CORNER')";
    ...
    try {
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            echo '<div class="text-center py-12 bg-gray-50 rounded-xl">
                    <h3 class="text-lg md:text-xl font-semibold text-gray-700 mb-2">No Corner articles found</h3>
