...
function fetchFile(fileName, fieldName) {
    ...
    fetch(`https://greek.asfirj.org/selectFile/?file=${encodeURIComponent(fileName)}`)
...
if (filename.startsWith("https://")) {
    ...
    url = `https://greek.asfirj.org/uploadedFiles/${filename}`
}

        fileContainer.innerHTML = ` <label for="${fieldName}">${label}:  </label>
                                <a href="${url}" style="margin-left:10px;" target=_blank > ${filename}</a> <button type="button" class="text-danger" style="margin-left:10px;"  onClick="removeFile('${fieldName}', '${fieldContainerId}', '${label}')"><i class="fa fa-trash"></i></button>`
    ...
    fetch(`https://greek.asfirj.org/backend/accounts/getArticleInfo.php`, {
        ...
        .then(async (data) => {
            if (data.success) {
                ...
                if (ArticleStatus === "saved_for_later" || ArticleStatus === "Saved" || ArticleStatus === "Drafted" || ArticleStatus === "returned_for_correction" || ArticleStatus === "correction_saved") {

                    previous_id.value = Article.article_id
                    ...
                    // gEt the authors 
                    fetch(`https://greek.asfirj.org/backend/accounts/articleAuthors.php?articleID=${articleId}`, {
                        method: "GET"
                    }).then(res => res.json())
                        .then(data => {
                            if (data) {
...
`
                    ...
                    uploadForm.addEventListener("submit", function (e) {
                        ...
                        fetch(`https://greek.asfirj.org/correction/`, {
                            method: 'POST',
                            body: formData
                        })
