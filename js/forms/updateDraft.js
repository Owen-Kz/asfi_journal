...
function AddFileField(fieldName, filename, fieldContainerId, label) {
    ...
    if (filename != null) {
        fileContainer.innerHTML = ` <label for="${fieldName}">${label}:  </label>
                                <a href="https://greek.asfirj.org/uploadedFiles/${filename}" style="margin-left:10px;" target=_blank > ${filename}</a> <button type="button" class="text-danger" style="margin-left:10px;"  onClick="removeFile('${fieldName}', '${fieldContainerId}', '${label}')"><i class="fa fa-trash"></i></button>`
...
`
                    ...
                    uploadForm.addEventListener("submit", function (e) {
                        ...
                        fetch(`https://greek.asfirj.org/draft/`, {
                            method: 'POST',
                            body: formData
                        })
